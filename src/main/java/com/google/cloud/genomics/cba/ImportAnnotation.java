...
public class ImportAnnotation {
	...
	public static interface Options extends ShardOptions {
		...
		@Description(
				"This provides the header for the Annotations. This is a required field. "
...
//		}
	...
	static class CreateAnnotations extends
		...
		public CreateAnnotations(String asId, String asType, String header, boolean is_0_Base, OfflineAuth auth, boolean write) {
			this.asId = asId;
			this.asType= asType;
			this.header=header;
			...
			for (String line : c.element().split("\n")) {
				if (!line.isEmpty() && !line.startsWith("#") && !line.startsWith("chrom")) {
					...
					if(asType.equalsIgnoreCase("GENERIC") && words.length >=3) {
						...
						.setReferenceName(canonicalizeRefName(words[0]))
						.setInfo(new HashMap<String, List<Object>>());
						
						if(this.is_0_Base){
							a.setStart(Long.parseLong(words[1]))
							.setEnd(Long.parseLong(words[2]));
						}
						else{ //1-Base
							a.setStart(Long.parseLong(words[1])-1)
							.setEnd(Long.parseLong(words[2]));
						}
						...
						//fields[3] or words[3] is REF, and comes from variant!!!
					...
					//TODO:At this point, our system only supports Generic and Variant Annotations
					else if(asType.equalsIgnoreCase("TRANSCRIPT") && words.length >=4) {
					
						Transcript t = new Transcript();
						t.setGeneId(words[3]);
						a.setType("TRANSCRIPT")
						.setAnnotationSetId(asId)
						.setReferenceName(canonicalizeRefName(words[0]))
						.setTranscript(t)
						.setInfo(new HashMap<String, List<Object>>());
		...
		/**
		 * <h1>This function remove "chr" from the beginning of referenceName (e.g, chr17 becomes 17)
		 * @param	refName 
		 * 				Reference Name 
		 * @return  refName
		...
		/**
		 * <h1>If the number of annotations in the last batch is less than the specified size of batch (e.g., 4096)
		 *  then this method post the last batch.
		 */ 
		@Override
